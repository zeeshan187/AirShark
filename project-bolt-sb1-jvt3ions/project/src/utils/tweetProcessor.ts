  ...
  return tweets.map(tweet => {
    try {
      ...
      return {
        ...
        source: tweet.source,
        score: tweet.score || 0,
        scoreReasons: tweet.scoreReasons || [],
      ...
      return {
        ...
        isQuote: false,
        score: 0,
        scoreReasons: ['Error processing tweet'],
        searchQuery: '',
        qualityIndicators: {
          hasDate: false,
          hasTime: false,
...
const extractTokenFromText = (text: string): string => {
  ...
  if (dollarTokenMatch) {
    return dollarTokenMatch[1].toUpperCase();
  }
  ...
  // If no token is found, return empty string instead of defaulting to SOL
  return '';
};
