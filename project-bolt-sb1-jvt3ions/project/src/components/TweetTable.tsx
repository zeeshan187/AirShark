...
type SortField = 'token' | 'date' | 'score' | 'likes' | 'retweets' | 'views';
...
const TweetTable: React.FC<TweetTableProps> = ({ 
  ...
  const filteredAndSortedTweets = useMemo(() => {
    ...
    let filtered = tweets.filter(tweet => {
      ...
      const matchesScore = tweet.score >= filters.minScore && 
        tweet.score <= filters.maxScore;
      ...
      return matchesToken && matchesScore && matchesDateFrom && matchesDateTo &&
            ...
            <div>
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  className="w-20 px-2 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:border-indigo-500"
                />
                ...
                <input
                  ...
                  onChange={(e) => setFilters(f => ({ ...f, maxScore: Number(e.target.value) }))}
                  className="w-20 px-2 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
        ...
        <table className="min-w-full divide-y divide-gray-700 bg-gray-800">
          <thead className="bg-gray-800">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('score')}>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-16 bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{
                          width: `${tweet.score}%`,
                          backgroundColor: tweet.score > 66 ? '#10B981' : tweet.score > 33 ? '#FBBF24' : '#EF4444'
                        }}
                      />
                    </div>
                    <span className="text-sm text-gray-300">{tweet.score.toFixed(1)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-4">
